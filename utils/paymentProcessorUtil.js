    ...
    var setupPools = function(enPools) {
        async.filter(enPools, function(coin, callback) {
            ...
            coins.forEach(function(coin) {
                ...
                logger.debug(logSystem, logComponent, 'Payment processing setup to run every ' + processingConfig.paymentInterval + ' second(s) with daemon (' + processingConfig.daemon.user + '@' + processingConfig.daemon.host + ':' + processingConfig.daemon.port + ') and redis (' + poolOptions.redis.host + ':' + poolOptions.redis.port + ')');
                    ...
                    setTimeout(function() {
                        ...
                        daemon.cmd('sendmany', [addressAccount || '', addressAmounts], function(result) {
                            ...
                            if (result.error && result.error.code === -6) {
                                ...
                                logger.error(logSystem, logComponent, 'Not enough funds to cover the tx fees for sending out payments, decreasing rewards by ' + (higherPercent * 100) + '% and retrying');
                                ...
                                logger.info(logSystem, logComponent, 'Sent out a total of ' + (totalSent / magnitude) + ' to ' + Object.keys(addressAmounts).length + ' workers');
                                if (withholdPercent > 0) {
                                    logger.error(logSystem, logComponent, 'Had to withhold ' + (withholdPercent * 100) + '% of reward from miners to cover transaction fees. ' + 'Fund pool wallet with coins to prevent this from happening');
                                }
                                callback(null, workers, rounds);
                            }
                        }, true, true);
                ...
                redisClient.multi(finalRedisCommands).exec(function(error, results) {
                    ...
                    if (error) {
                        ...
                        logger.error(logSystem, logComponent,
                            'Payments sent but could not update redis. ' + JSON.stringify(error) + ' Disabling payment processing to prevent possible double-payouts. The redis commands in ' + coin + '_finalRedisCommands.txt must be ran manually');
